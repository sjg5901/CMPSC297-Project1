...
const crypto = require("crypto"); // Used for encryption algorithms; Build-in
...
class Block {
    constructor(prevHash = "", transactions = []) {
        ...
        this.nonce = 0; // random value for mining purposes
    ...
    getHash() {
        ...
        return SHA256(
            ...
            // Previous hash + the timestamp of the block + all transactions + some random nonce for fun
            // jumbles everything into 256 bit hash
    ...
    // A block that is mined is only value if the first x bits are zero

    // Mine a new block (generaate a hash that works)
